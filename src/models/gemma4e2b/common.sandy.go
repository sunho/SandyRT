...
func gemma_local_eval_kv_attention(x Tensor, position_id Tensor, k_cache Tensor, v_cache Tensor) Tensor {
    weight_scope "self_attn" {
        ...
        k = __reshape(k, shape=[-1, -1, 1, 256])
        v = __reshape(v, shape=[-1, -1, 1, 256])
        ...
        scores := __sliding_query_key_score(q, k_cache, position_id, window=512, scale=1.0)
        probs := __softmax(scores, dim=-1)
        ctx := __matmul(probs, v_cache)
...
func gemma_global_eval_kv_attention(x Tensor, position_id Tensor, k_cache Tensor, v_cache Tensor) Tensor {
    weight_scope "self_attn" {
        ...
        k = __reshape(k, shape=[-1, -1, 1, 512])
        v = __reshape(v, shape=[-1, -1, 1, 512])
        ...
        scores := __sliding_query_key_score(q, k_cache, position_id, window=0, scale=1.0)
        probs := __softmax(scores, dim=-1)
        ctx := __matmul(probs, v_cache)
...
func gemma_local_eval_attention(x Tensor, position_id Tensor, k Tensor, v Tensor) Tensor {
    weight_scope "self_attn" {
        ...
        scores := __sliding_query_key_score(q, k, position_id, window=512, scale=1.0)
        probs := __softmax(scores, dim=-1)
        ctx := __matmul(probs, v)
...
func gemma_global_eval_attention(x Tensor, position_id Tensor, k Tensor, v Tensor) Tensor {
    weight_scope "self_attn" {
        q := __matmul(x, __transpose(@q_proj.weight))
        q = __reshape(q, shape=[-1, -1, 8, 512])
        q = __permute(q, dims=[0, 2, 1, 3])
        ...
        scores := __sliding_query_key_score(q, k, position_id, window=0, scale=1.0)
        probs := __softmax(scores, dim=-1)
        ctx := __matmul(probs, v)
...
func gemma_local_eval_layer(x Tensor, per_layer_input Tensor, position_id Tensor, i int, k Tensor, v Tensor) Tensor {
    weight_scope "layers.{i}" {
        h := __rms_norm(x, @input_layernorm.weight)
        h = gemma_local_eval_attention(h, position_id, k, v)
        h = __rms_norm(h, @post_attention_layernorm.weight)
        x = __add(x, h)
...
func gemma_global_eval_layer(x Tensor, per_layer_input Tensor, position_id Tensor, i int, k Tensor, v Tensor) Tensor {
    weight_scope "layers.{i}" {
        h := __rms_norm(x, @input_layernorm.weight)
        h = gemma_global_eval_attention(h, position_id, k, v)
        h = __rms_norm(h, @post_attention_layernorm.weight)
        x = __add(x, h)
...
func gemma4e2b_model(input_ids Tensor) Tensor {
