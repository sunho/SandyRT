...
def main() -> int:
    root = repo_root()
    parser = argparse.ArgumentParser(description="Prepare and run Sandy Gemma4_E4B text-only BF16 chat.")
    ...
    if args.keep_input:
        ...
        fd, tmp_name = tempfile.mkstemp(prefix="sandy_gemma4e4b_input_", suffix=".safetensors")
        os.close(fd)
