...
TEST_F(InvocPlannerTest, ReshapeIsZeroCopyAlias) {
    sandy::ir::mid_ir::Graph graph;
    sandy::ir::mid_ir::Builder builder(graph);
    ...
    EXPECT_TRUE(draft.programSources.empty());
    ASSERT_EQ(draft.outputs, std::vector<sandy::engine::InvocValueId>({0}));
    ASSERT_EQ(draft.instructions.size(), 2u);
...
TEST_F(InvocPlannerTest, ReshapeFeedsComputeWithoutIndependentProgram) {
    ...
    EXPECT_EQ(draft.instructions[3].kind, sandy::engine::InvocInstructionKind::Dealloc);
    EXPECT_EQ(draft.instructions[4].kind, sandy::engine::InvocInstructionKind::StoreOutputs);
